...
- **Always** use `overrides` in the naming map when a name means different things in different periperhals/registers.
- **Never** use C/C++ keywords as the name of a variable or type. No type or variable can be called `CLASS` or `class`, for example.
...
- **Always** to make variables in the union lower case and used _to separate words. For example: `hsienable` => `high_speed_internal_enable`. This is to make it clear that it's a variable and not a type, and to make it more readable. Types should be in CamelCase and variables should be in lower snake_case. The only exception is for existing acronyms which are commonly used in the industry like `AHB` or `APB`, which should be left as is (this is due to the expansions being too long and not adding much value, and the existing acronyms being well known in the industry).
...
- **Prefer** to change some acryonyms to single words like:
  - `CPU` => `Processor`
  - `RAM` => `Memory`
  - `ETH` => `Ethernet`
  - `PHY` => `Physical`
