...
def validate_structure(structure : typing.Dict[str, str]) -> None:
    ...
    Args:
        structure (Dict) The dictionary describing the structure
    """
...
def pad_members(members : typing.List[typing.Dict[str,str]], default_type: str, member_sizeof: int, start : int, limit : int) -> int:
...
def pack_members(old_members : typing.List[typing.Dict[str,str]], depth: int, default_type: str, sizeof: int) -> typing.List[typing.Dict[str,str]]:
...
def pad_fields(fields : typing.List[typing.Dict[str,str]], default_type: str, depth : int, start: int, limit: int) -> int:
...
def pack_fields(old_fields : typing.List[typing.Dict[str, str]], depth : int, default_type: str, sizeof : int) -> typing.List[typing.Dict[str, str]]:
...
def process_enums(top : typing.Dict[str, str]) -> None:
    if "enums" in top:
        ...
        for yml in top["enums"]:
            ...
            # data = sorted(data, key=lambda e: e["value"])
            if verbose:
...
def process_register(top: typing.Dict[str, str]) -> None:
    if "registers" in top:
        ...
        for yml in top["registers"]:
            ...
            reg["fields"] = pack_fields(old_fields=reg["fields"], depth=int(reg["default_depth"]), default_type=reg["default_type"], sizeof=int(reg["sizeof"]))
...
def process_structure(top: typing.Dict[str, str]) -> None:
    if "structures" in top:
        ...
        for yml in top["structures"]:
            ...
            sub["members"] = pack_members(old_members=sub["members"], depth=int(sub["default_depth"]), default_type=sub["default_type"], sizeof=int(sub["sizeof"]))
...
def main(argv : typing.List[str]) -> int:
    ...
    parser.add_argument('-o', "--output", type=str, action="store", default=os.getcwd(), help="[optional] the output path if given (default:%(default)s)")
    parser.add_argument('-tr',"--template-root", type=str, action="store", default=os.path.join(os.getcwd(), "templates"), help="The location where the templates are kept (default=%(default)s)")
    parser.add_argument('-yr',"--yaml-root", type=str, action="store", default=None, help="The root to the location where the yamls are kept")
...
""")
    ...
    for yml in args.yaml:
        ...
        peripheral["members"] = pack_members(old_members=peripheral["members"], depth=depth, default_type=default_type, sizeof=sizeof)
